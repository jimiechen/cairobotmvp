...
// 使用 SQLite 内存数据库验证全部 22 个 Repository 方法的正确性
...
// setupTestDB 初始化 SQLite 内存数据库并执行 AutoMigrate
// 每个测试函数调用获得独立的空数据库，测试间完全隔离
func setupTestDB(t *testing.T) *gorm.DB {
	t.Helper()
	db, err := gorm.Open(sqlite.Open(":memory:"), &gorm.Config{})
	require.NoError(t, err)
	require.NoError(t, db.AutoMigrate(&Group{}, &GroupMember{}, &GroupPayConfig{}, &GroupStats{}))
	return db
}
...
func TestCreateAndGetGroup(t *testing.T) {
	...
	group := makeTestGroup("g001", "Go技术圈", "go-tech", "user001")
	require.NoError(t, repo.CreateGroup(ctx, group))

	// 按 ID 查回，字段一致
	found, err := repo.GetGroupByID(ctx, "g001")
	require.NoError(t, err)
	...
	require.Equal(t, "g001", foundBySlug.ID)
...
func TestUpdateGroup(t *testing.T) {
	...
	group := makeTestGroup("g002", "原始名称", "orig-slug", "user001")
	...
	updated, _ := repo.GetGroupByID(ctx, "g002")
...
func TestDeleteGroup(t *testing.T) {
	...
	group := makeTestGroup("g003", "待删除", "del-slug", "user001")
	repo.CreateGroup(ctx, group)

	require.NoError(t, repo.DeleteGroup(ctx, "g003"))
	deleted, err := repo.GetGroupByID(ctx, "g003")
	require.NoError(t, err)
...
func TestListGroupsByOwnerID(t *testing.T) {
	...
	repo.CreateGroup(ctx, makeTestGroup("g010", "A的群", "a-group", "owner_a"))
	repo.CreateGroup(ctx, makeTestGroup("g011", "B的群", "b-group", "owner_b"))
	repo.CreateGroup(ctx, makeTestGroup("g012", "A的第二个群", "a-group2", "owner_a"))
...
func TestCreateAndGetMemberByGroupAndUser(t *testing.T) {
	...
	member := makeTestMember("m001", "g001", "u001", GroupMemberRoleMember)
	require.NoError(t, repo.CreateMember(ctx, member))

	// 按 groupID + userID 联合查询
	found, err := repo.GetMember(ctx, "g001", "u001")
	require.NoError(t, err)
	require.NotNil(t, found)
	require.Equal(t, "m001", found.ID)
	require.Equal(t, GroupMemberRoleMember, found.Role)

	// 按 ID 查询
	foundByID, err := repo.GetMemberByID(ctx, "m001")
	require.NoError(t, err)
	require.NotNil(t, foundByID)
	require.Equal(t, "g001", foundByID.GroupID)
}
...
func TestCreateGroupWithOwnerMember(t *testing.T) {
	...
	group := makeTestGroup("g100", "带Owner的群", "with-owner", "owner_001")
	require.NoError(t, repo.CreateGroup(ctx, group))

	ownerMember := makeTestMember("m_owner_100", "g100", "owner_001", GroupMemberRoleOwner)
	require.NoError(t, repo.CreateMember(ctx, ownerMember))

	// 验证 owner 成员可被查询到
	isMember, err := repo.IsUserMember(ctx, "g100", "owner_001")
	require.NoError(t, err)
	require.True(t, isMember, "创建者加入为群主后应被识别为活跃成员")

	// 验证角色正确
	member, err := repo.GetMember(ctx, "g100", "owner_001")
	require.NoError(t, err)
...
func TestUpdateAndDeleteMember(t *testing.T) {
	...
	member := makeTestMember("m002", "g001", "u002", GroupMemberRoleMember)
	repo.CreateGroup(ctx, makeTestGroup("g001", "测试群", "test-g", "owner_x"))
	...
	updated, _ := repo.GetMemberByID(ctx, "m002")
	require.Equal(t, GroupMemberStatusBanned, updated.Status)

	// 删除成员
	require.NoError(t, repo.DeleteMember(ctx, "m002"))
	deleted, err := repo.GetMemberByID(ctx, "m002")
	require.NoError(t, err)
	require.Nil(t, deleted)
}
...
func TestListMembersWithPagination(t *testing.T) {
	...
	repo.CreateGroup(ctx, makeTestGroup("g200", "分页测试群", "page-test", "owner_x"))
	...
	list, total, err := repo.ListMembers(ctx, "g200", 1, 3, nil, nil)
	require.NoError(t, err)
	require.Equal(t, int64(7), total)
	require.Len(t, list, 3)

	// 带 role 过滤：只查管理员（当前无管理员）
	adminList, adminTotal, _ := repo.ListMembers(ctx, "g200", 1, 10, ptrInt8(GroupMemberRoleAdmin), nil)
	require.Equal(t, int64(0), adminTotal)
	require.Empty(t, adminList)

	// 带 status 过滤
	activeList, activeTotal, _ := repo.ListMembers(ctx, "g200", 1, 10, nil, ptrInt8(GroupMemberStatusActive))
...
func TestIsUserMember(t *testing.T) {
	...
	repo.CreateGroup(ctx, makeTestGroup("g300", "身份检查群", "identity-check", "owner_x"))
	repo.CreateMember(ctx, makeTestMember("m300", "g300", "active_user", GroupMemberRoleMember))
	repo.CreateMember(ctx, makeTestMember("m301", "g300", "banned_user", GroupMemberRoleMember))

	// 将 banned_user 设为封禁状态
	banned := &GroupMember{ID: "m301", Status: GroupMemberStatusBanned}
	repo.UpdateMember(ctx, banned)

	// 活跃成员 → true
	isActive, err := repo.IsUserMember(ctx, "g300", "active_user")
	require.NoError(t, err)
	require.True(t, isActive)

	// 封禁成员 → false（status != 1）
	isBanned, err := repo.IsUserMember(ctx, "g300", "banned_user")
	require.NoError(t, err)
	require.False(t, isBanned)

	// 非成员 → false
	isNonMember, err := repo.IsUserMember(ctx, "g300", "stranger")
	require.NoError(t, err)
...
func TestCountActiveMembersAndByRole(t *testing.T) {
	...
	repo.CreateGroup(ctx, makeTestGroup("g400", "统计测试群", "stats-test", "owner_x"))

	// 插入混合角色的成员
	repo.CreateMember(ctx, makeTestMember("m400", "g400", "owner", GroupMemberRoleOwner))
	repo.CreateMember(ctx, makeTestMember("m401", "g400", "admin1", GroupMemberRoleAdmin))
	repo.CreateMember(ctx, makeTestMember("m402", "g400", "admin2", GroupMemberRoleAdmin))
	repo.CreateMember(ctx, makeTestMember("m403", "g400", "member1", GroupMemberRoleMember))

	// 活跃成员数 = 4
	activeCount, err := repo.CountActiveMembers(ctx, "g400")
	require.NoError(t, err)
	require.Equal(t, int64(4), activeCount)

	// 管理员数 = 2
	adminCount, err := repo.CountMembersByRole(ctx, "g400", GroupMemberRoleAdmin)
	require.NoError(t, err)
	require.Equal(t, int64(2), adminCount)

	// 群主数 = 1
	ownerCount, err := repo.CountMembersByRole(ctx, "g400", GroupMemberRoleOwner)
	require.NoError(t, err)
...
func TestListMembersByUserID(t *testing.T) {
	...
	repo.CreateGroup(ctx, makeTestGroup("g500", "群A", "ga", "owner_x"))
	repo.CreateGroup(ctx, makeTestGroup("g501", "群B", "gb", "owner_y"))

	repo.CreateMember(ctx, makeTestMember("m500", "g500", "multi_user", GroupMemberRoleMember))
	repo.CreateMember(ctx, makeTestMember("m501", "g501", "multi_user", GroupMemberRoleAdmin))
...
func TestPayConfigCRUD(t *testing.T) {
	...
	// 查询
	found, err := repo.GetPayConfigByGroupID(ctx, "g600")
	require.NoError(t, err)
	require.NotNil(t, found)
	...
	updated, _ := repo.GetPayConfigByGroupID(ctx, "g600")
...
func TestGetOrCreateStatsAndUpdateStats(t *testing.T) {
	...
	// 首次获取 → 自动创建默认值
	stats, err := repo.GetOrCreateStats(ctx, "g700")
	require.NoError(t, err)
	require.NotNil(t, stats)
	require.Equal(t, "g700", stats.GroupID)
	require.Equal(t, int64(0), int64(stats.MembersCount))

	// 再次获取 → 返回已有记录（幂等）
	stats2, err := repo.GetOrCreateStats(ctx, "g700")
	require.NoError(t, err)
	...
	updated, _ := repo.GetOrCreateStats(ctx, "g700")
