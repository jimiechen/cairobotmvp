...
type Servant struct {
	handler   *Handler
	jwtMgr   *JWTManager  // 用于从 token 提取 jti 做黑名单检查
	tokenStore TokenStore // 黑名单存储（可能为 nil，nil 时跳过检查）
}
...
//
// 鉴权路径黑名单检查（§8.2.1）：
//   在写入 CtxKeyUserID 之前，通过公共函数 CheckTokenBlacklist 检查 token 的 jti 是否在黑名单中。
...
func (s *Servant) Handle(ctx context.Context, req []byte, extend map[string]string) (int, []byte, error) {
	...
	if code, respBytes, err := CheckTokenBlacklist(ctx, extend, s.jwtMgr, s.tokenStore); code != 0 {
